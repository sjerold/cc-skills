...
多Agent协作系统，采用大唐贞观年间历史人物命名，支持任意类型任务的多模态处理。

**架构**：
- **三公（决策层）**：太师房玄龄（规划）、太傅魏征（审核）、太保长孙无忌（调度）
- **六部（执行层）**：吏部裴行俭（状态）、户部刘晏（输入）、礼部贺知章（输出）、兵部李靖（分析）、刑部狄仁杰（审核）、工部阎立德（代码）
...
### tang-dynasty v1.0.0 (2026-04-02)
- 首次发布
- 三公六部多Agent协作架构
- 支持任意类型任务的多模态处理
- 分层决策与执行机制
