...
> **图片描述**: 50/54 张已生成 img2txt 文字描述（括号内）,便于非多模态模型解析
...
![图表展示了2025年AI编程工具市场格局,按L1-L5能力层级分类](images/img_00.webp)

（图片描述：图表展示了2025年AI编程工具市场格局,按L1-L5能力层级分类）

后续内容...
```

**图片描述的两种呈现**：
1. 写在 `![]` 的 alt 位置：`![描述文字](images/xxx.webp)`
2. 在图片下方单独一行：`（图片描述：描述文字）`

这样无论是多模态模型(看图)还是纯文本模型(GLM 读 alt 或正文)都能获取图片信息。
