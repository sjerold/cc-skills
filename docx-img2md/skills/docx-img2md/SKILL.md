...
| **直接用Python调用OCR** | 禁止用PowerShell/ps1包装；Agent直接用 `cmd //c "call conda activate dsbot_env && python external_ocr.py --images <pic> --output <txt>"` 调用，`--output` 参数直接写UTF-8文件，彻底绕过PowerShell GBK编码问题 |

**禁止**：修改txt文字内容、自行设置SP_TOKEN环境变量、用脚本/后台任务代替Agent跑OCR、用脚本做md内容智能处理（机械拼接part片段除外）、用PowerShell/ps1包装OCR调用
...
**Agent 直接用 Python 调用 OCR**（不用 PowerShell/ps1）：

```bash
cmd //c "call conda activate dsbot_env && python <external_ocr.py路径> --images <pic路径>\image_XXX.png --output <txt路径>\image_XXX.txt"
```

关键点：
- `--output` 参数让 external_ocr.py 直接写 UTF-8 文件，不经过 PowerShell Set-Content，**彻底消除中文路径乱码**
- `--images` 每次只传1张图
- Agent 用 Bash 工具执行上述命令，检查退出码和输出文件

**Agent 自修复职责（强制）**：
- 每张图执行后，Agent 必须检查：① 退出码为0；② 输出 txt 文件存在且大小 > 30字节。
- 任一不满足即失败（常见原因：讯飞秒级流控 11202、网络抖动、单张超时、**503 系统繁忙**）。
- 失败时 Agent 必须**立即重试该张**，重试间隔1秒，直到成功才处理下一张。**不得跳过、不得记 FAIL 继续、不得留给主线程事后补跑。**
- 遇 **503 持续错误**：暂停5秒后重试当前张（网关瞬时繁忙，等一会就好），不要因 503 放弃。
...
#### 3.4 边界续表处理（强制）
- Agent 开头若发现首个 txt 是上一区间的续表行（无表头），先 Read 上区间最后一个 txt 看 txt 看末尾是什么表，按续行处理并标注 `> 注：image_XXX 疑似承接 image_YYY 续表`。
...
OCR 调用的三个配置项均从环境变量读取（Agent 调用时无需传参，由用户预配置环境变量）：

| 环境变量 | 作用 | 默认值 | 命令行参数（覆盖环境变量） |
|----------|------|--------|--------------------------|
| `SP_TOKEN` | API Key（必填） | 无 | `--token` |
| `SP_API_BASE` | API 入口地址 | `https://maas-coding-api.cn-huabei-1.xf-yun.com/v2`（讯飞 MaaS） | 无 |
| `SP_MODEL` | 模型名称 | `xopkimik26` | `--model` |

- `$env:SP_TOKEN` / `SP_API_BASE` / `SP_MODEL` 均由用户预配置，模型禁止修改
- 切换 LLM 供应商或套餐时，改对应环境变量即可，无需改代码或 skill 调用命令
